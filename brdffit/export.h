...
void writeTabC(mat3 * tab, vec2 * tabAmplitude, int N)
{
	...
	//file << "static const int size = " << N  << ";" << std::endl << std::endl;
	...
	{
		//file << "{";
		file << tab[a + t*N][0][0] << ", " << tab[a + t*N][0][2] << ", ";
		file << tab[a + t*N][1][1] << ", ";
        file << tab[a + t*N][2][0]; //<< "}";
		if(a != N-1 || t != N-1)
			file << ", ";
	...
	file << "static const float tabAmplitude[size*size] = {" << std::endl;
	for(int t = 0 ; t < N ; ++t)
	for(int a = 0 ; a < N ; ++a)
	{
		file << tabAmplitude[a + t*N][0] << ",0,0,0";
		if(a != N-1 || t != N-1)
			file << ", ";
		file << std::endl;
	}
...
void writeTabMatlab(mat3 * tab, vec2 * tabAmplitude, int N)
{
	...
	file << "# name: tabAmplitude" << std::endl;
